class CartWithConditionItems extends HTMLElement {
  constructor() {
    super();
  ...
  /**
   * Checks if cart conditions were violated and changes quantity if they weren't.
   * onChange event listener for quantity inputs in cart
   * @param {Event} event 
   * @returns {Promise} promise
   */
  async onChange(event) {
    let checkCartCondition = new CheckCartCondition({
      maxTotalProductsQuantity: this.getAttribute('max_total_products_quantity'),
      maxEachProductQuantity: this.getAttribute('max_each_product_quantity'),
      maxCartTotal: this.getAttribute('max_cart_total'),
    });

    let hasCartConditionBeenViolated = await checkCartCondition.isViolatedCartPage({
      productIndex: parseInt(event.target.dataset.index),
      productQuantity: event.target.value - event.target.defaultValue
    });
   ...
   * @param {string} line product index in cart
   * @param {number} quantity new quantity
   * @param {string} name name attribute for active element
   */
...
customElements.define("cart-with-condition-items", CartWithConditionItems);
